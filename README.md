...
- 红绿灯按钮动画：关闭（红）→ 居中缩小淡出；最小化（黄）→ easeSuction 吸出 Dock；最大化（绿）→ easeMorph 平滑展开；中间插入 4% 缩放峰值
- 启动屏：Apple Logo 弹性进入 → 进度条加载 → 光晕爆发 → 淡出过渡（强制最低 3 秒）
- **首次设置向导**：欢迎页 → 选择壁纸 → 选择主题（深浅色）
- 顶部菜单栏：应用名实时同步、WiFi / 电池状态图标、动态菜单
- 桌面壁纸渐变、桌面图标双击打开应用
